...
def test_uv_pip_list():
    ...
    try:
        ...
        # Check for core dependencies in output
        output = result.stdout.lower()
        expected_packages = ['flask', 'sqlalchemy', 'flask-socketio']
        found_packages = [pkg for pkg in expected_packages if pkg in output]
        
        if len(found_packages) >= 2:
            print(f"✅ UV pip list shows {len(found_packages)} core packages")
            return True
        else:
            print(f"❌ UV pip list missing core packages, found: {found_packages}")
            return False
            
    except (subprocess.CalledProcessError, FileNotFoundError):
        print("❌ 'uv pip list' command failed")
        return False
